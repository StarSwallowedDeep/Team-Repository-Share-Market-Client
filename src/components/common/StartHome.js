...
import { writeAccess } from "../../lib/api/access";
...
const StartHome = () => {
  ...
  const saveDataToMongoDB = async () => {
    try {
      await writeAccess({ device: userAgent, location: `${latitude}, ${longitude}` });
      ...
      <StartHomeBlock isVisible={isVisible} delay={2}>
        <Link to="/postlist">
          <div className="button-wrapper">
            <button onClick={saveDataToMongoDB} to="/postlist">시작하기</button>
          </div>
        </Link>
      </StartHomeBlock>
    </StartHomeWrapper>
